    ...
    items.forEach((item) => {
        ...
        div.innerHTML = '<div class="card-header">' +
            ...
            '<div class="bb-info-avatar">'+'<span>'+'<div class="name">' + item.author.nickName + '</div>'+ svg +'</span>' + '<div class="card-time">' + time+'</div>'
              + '</div></div><div class="card-content">' + item.content + '</div><div class="card-footer"><div data-v-185689ea=""class="card-label"style="background: ' + item.tag.bgColor + '; color: white;">' + item.tag.name + '</div><div class="bb-fos"><svg style="width:1.60em;height:1.60em;fill:var(--theme-color);overflow:hidden;cursor: pointer;"><use xlink:href="#icon-xiaoxi"></use></svg></div></div>'
        bb.appendChild(div)
    })
    // waterfallLayout(bb);
}
...
function contentFormat(s) {
    ...
    if (ls) {
        ls.forEach((e) => {
            html += '<a href="' + e + '" target="_blank" data-fancybox="group" class="fancybox"><img src="' + e + '" data-lazy-src="'+ e +'" style="max-width: 100%; object-fit: cover" class="bb-Img entered loaded" data-ll-status="loaded"></a>'
        })
    }
    s += html
    return s
