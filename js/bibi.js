...
function waterfallLayout(bb) {
    ...
    cards.forEach((card, index) => {
        ...
        card.style.left = `${minHeightColumnIndex * (card.offsetWidth + 12)}px`; // 10px 是列间距

        // 更新该列的高度
        columnHeights[minHeightColumnIndex] += card.offsetHeight + 12 // 20px 是卡片之间的间距
    });
    const maxHeight = Math.max(...columnHeights);
    bb.style.height = `${maxHeight + 12}px`;
}
